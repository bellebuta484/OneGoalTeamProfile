...
When forming an IT group and collaboration is very complicated, and the lack of implementations. However, GitHub and Git provide a platform for teams and individuals to create a homepage and communicate, revise, and chat on the website. In this assignment, Git repository has reflected our group's work in so many ways, such as we are able to share our thoughts, edits and comments on the group Git repository. Furthermore, we can track the progress of our group's work and provide an appraisal. 

## Industry Data 

![](images/Ideal-Job-Image-1-900x506.jpeg)
...
+ Banta Co. Creative Directors and Designers. Vishal is the sole developer for their web site, and as the company is relatively small and a start up he answers directly to the head designers and creative directors of the company. Vishal is responsible of creating a website as close as possible to their design model, this is subject to review at any time and often changes are required adhoc and with little notice. They work closely with others within the company for verbal feedback on the aesthetics of the website, but they alone are the key stakeholders. The other members are simply included for a “fresh set of eyes”. 

Both the head designer and the creative director have authority to pause the process at any stage to prevent it from going too far into a direction that they are not comfortable with. Systematic reviews of the work being done helps reinforce that he is on the right track with the web site he is designing.   

### Question 3: Time allocation 
+ Most days are spent Coding using different languages (e.g. HTML, PHP, JQuery) however in the context of a specific project of introducing the new website prior to launch, only four weeks were allocated from start to “completion”. The reason completion comes in quotations is that, in Vishal's words, it is an ongoing process that he feels grows with user needs and response. So essentially from initial planning to launch is four weeks however there is extensive monitoring, tweaking and redesigning needed in the weeks following the launch as user engagement grows and user patterns emerge. The time immediately following a launch also requires a lot of work. 

Initial planning, discussion and design took place over two weeks to complete. Planning and reviewing ideas thoroughly to save time on coding in the weeks to come. Once design had been agreed upon by designers, the building of the wireframe followed. Also best described as the blueprint or framework for the website. This took roughly one week to complete as similar aspect were able to be used from the previous work. With the base structure in place, the next step was to begin the base coding.  This task had a two-week allocation but due to his experience (and some extra hours of Overtime) he was able to complete it within one week.   

Now that the website has launched officially, they continue to review and evaluate the data from the traffic received on the website. This means the process is still ongoing for inner pages and updates, always trying to improve on their initial work.   

### Question 4: Challenging Aspects	 
+ Vishal noted the foremost aspect that challenges a web designer most in his opinion was the ability to ensure cross compatibility between the multitude of devices that are available to us in our everyday lives without having to make major sacrifices to the UI/UX. Another challenge is ensuring that he can completely understand what the designers are after and translating their vision into code. Not only successfully producing code to satisfy their wants but doing it in a way that is still able to create a positive user interaction too. In creating the new website, aspect from the previous site were carried over. This was more than just a cut and paste as it lowered the overall efficiency and required recoding to implement effectively. 
...
In order to support Bitcoins peer-to-peer transactional concept, Nakamoto developed a revolutionary technology known as Blockchain (Zhao, Fan and Yan, 2016, p1). A blockchain is a decentralized digital transaction ledger, maintained by a network of computers that do not rely on an intermediary or third party. Individual transaction data files (blocks) are created to include all data and have specific hash ID’s that include previous blocks Hash IDs so as to highlight their position in the chain. These Blocks are managed through specific software platforms that allow the data to be transmitted, processed, stored, and represented in human readable form (Kamilarisa, Fonts and Prenafeta-Boldύ, 2019, p640).  

The main idea of blockchain is a distributed database of records of transactions that are shared across participating parties. Every transaction is verified by the consensus of a majority of the participants in the system using various algorithms, making fraudulent transactions unable to pass verification. Once a record is created and accepted by the blockchain, it can never be altered (Zhao, Fan and Yan, 2016, p2). 
...
Legal Issues: At present most nations do not have legislation that permits vehicles beyond a level 3. Legal issues are at the centre of the challenges to rolling our fully Autonomous vehicles including insurance liabilities however it will be impossible to predict all legal risks until more advanced forms of technology are available. (Channon, McCormick and Noussia, 2019, p3) 
...
Further, future advances can be found in the development of defensive Cyber Security technologies by nation states and corporations. (Forbes, 2021).  This is due to the ever, increasing threats from unknown actors through the internet.  The advancements within these defensive technologies will allow for automated systems that do not require human analysts to mitigate security threats and the inclusion of predictive systems that can predict what future attacks will be and mitigate them effectively (Forbes, 2021). Furthermore, the advancements within this area will lead to defensive technologies that are multi-layered and integrated within systems. However, legacy systems will have to be upgraded in order to ensure compatibility with these new defensive technologies (Threat Hunting, 2021).  

**Future Impacts** 

With continuous advances in the Cyber Security space there will be several impacts upon jobs, and current technologies used by companies and nations. Furthermore, these impacts will affect people working in a Cyber Analyst role the most. This is because the requirements for these job roles will change as advancements in security technologies evolve (Lauterborn, 2021). Furthermore, advances in Cyber Security technologies will affect jobs, which is due to current and continuing advances in Artificial Intelligence. This is because Artificial Intelligence will be able to fully automate the entire Cyber Security process. This includes, threat detection, threat mitigation techniques, analysis of all systems on the network for security flaws and prediction of future attacks (Azvine, 2021). Thus, Artificial Intelligence will affect jobs such as ‘Cyber Analyst’ as these roles will no longer be required as Artificial Intelligence becomes more advanced. 
...
My current project Idea is to create an all-in-one fitness app that allows people to track and monitor their fitness goals. This app will provide users with advice on workouts and track their calorie intake.  The workout advice contained will be in the form of tailored, custom workout plans. The app will track fitness goals by storing information such as, weights that the user is lifting, repetitions of each exercise, and the number of sets. These will also be customisable if the user does not wish to use the default amounts in the workout plan. 
 
### Motivation 

Even though there are many fitness apps and tracking apps available on smartphones, many either have missing features or require that users download additional ‘companion apps’ to gain use of all the features that they require. An example of such can be seen with the Map My Run application available for iOS and Android (‎Under Armour, 2021). This application allows for tracking of running however, do not offer anything in the way of caloric goal setting or custom plans based on the user and their fitness level. My motivation in creating this product is to ensure that the user has a streamline, and simple experience that does not require any additional apps or monthly subscriptions. Thus, making it more likely that users will continue to engage with the app and complete their fitness goals. 
...
Furthermore, the calorie tracking component allows for the user to quickly enter the foods that they are eating. This assists the app in ensuring that the user is tracking to achieving their fitness goals. This is because if a user is looking to lose weight then they will need to ensure that they are maintaining a caloric deficit. However, if a user is looking to build muscle and gain weight then they will need to ensure that they do the opposite.  This component will be able to adapt itself to user needs by giving the user a calorie goal that they will need to meet for them to achieve their goals.   

Moreover, in future iterations of the application there will be features such as automatic exercise detection available or the ability to quick select this from the ‘Home Screen’. This will allow for users to save time in the setup of the application and begin exercising.   
 
### Tools and Tech

In order to fully develop this project, tools and technologies from Apple and Google will be required in order to bring this App to both the Apple and Google ecosystems. These tools are in the forms of Standard Development Kits for both mobile platforms. This consequently requires computers that are capable for developing on these platforms. Especially in the case of developing for Apple Devices since an Apple computer is required in order to get full development support from Apple in the form of XCode (Apple, 2020). As a result, knowledge of the Objective C language or Swift is required to fully develop for this platform. Moreover, in order to produce this application on the Google Android ecosystem Google Studio will be required and as well as knowledge of Java and XML for programming the User Interface (Google, 2020). Furthermore, tools such as MongoDB will also be required as this tool provides an easy solution in creating Data Bases that store user information for use with this application (MongoDB, 2020).  

Moreover, these technologies work together to create the application by compiling and linking all the project files. This is done through the Integrated Development environment such as XCode or Google Studio. This then allows for the testing and then deployment of the app onto the target app stores which are Apple’s AppStore and Google’s Play Store. Furthermore, this application will utilise programming languages to the create the User Interface and backend for the application (backend meaning the behind the scenes processing of the data that is then used later).  Moreover, the application will then interact with MongoDB by having used the programming language to program the application to use this technology to read and write user data.   
 
### Skills Required 

The skills required to fully implement this project is experience in Programming within Java, Objective-C, Swift and XML mark-up languages.  This is because this project is aimed to roll out across two different mobile platforms. Thus, the project will have to be written in the programming languages for each of these platforms (i.e., Objective-C and Swift for iOS and Java and XML for Android) (Linkedin, 2021). Moreover, knowledge of both Integrated Development Environments (IDE) is required as well in order to produce the app. 

Furthermore, Skills within animation, video editing, photo editing and User Interface are a requirement for successfully completing this project.  This is because this product is designed to be interactive and without these skills the project cannot be fully created to the given specification outlined in the description (Ascendle, 2021). 

Moreover, skills within Data Base creation and security are required in order to store user data on the mobile devices.  This is due to both ecosystems requiring developers to ensure that their apps store user data in a secure and efficient manner (Apple, 2021). 

### Outcome

Upon the successful completion and distribution of this application this will solve a huge dilemma for consumers when it comes to fitness applications. This dilemma being that many other fitness solutions are fragmented requiring that users utilise additional companion applications. This application will also tie together many missing features that have been promised within other applications.  Furthermore, this development will have significant impacts upon the ‘app market’ in that, developers will have to increase their quality of products to match this one in order continue to remain competitive (Appinventiv, 2021). However, this may also result in other ‘clone’ applications that attempt to mimic the behaviours of the application was produced. Moreover, distribution will occur by uploading the finished, compiled, application through the IDEs to the respective app stores. However, Apple does require that all applications submitted through their IDE undergo analysis by them and also comply with their regulations. Thus, this may cause some delays in producing a successful outcome (Softensy, 2021).   
...
The team project was started at the start of the new year and was progressing well considering the loss of 2 team members during the early stages. This project thanks to members of the team was easy to organise, goals were easily defined and achieved, progressed as expected even though people left. Further, through a high level of communication everyone was able to maintain momentum and clarify doubtful points that would have otherwise affected the performance of the group.  However, some challenges we faced especially in the beginning was the organisation for meeting times. This was because all team members lived in different time zones. This was mitigated through MS Teams’ calendar which was able to easily translate times into each team members locality so that a time could be reached in which everyone was available. If, however, they were not able to make it that day we had all the recordings available over the team chat and members could also, type any questions they had within and gain clarity quickly. Overall, through communication and planning this project was easily completed by the team to an excellent standard.  

*Kristie Magazinovic*

The team kicked the project off in the new year however suffered a few setbacks early, loosing 2 team members in the first week of team meetings. I have worked on many group projects before (being at the end of my degree) and they are often extremely difficult to navigate. This is often due to conflicting ideas and vastly varying levels of commitment (or more notably a lack of commitment in many team members); however, this team has worked brilliantly and has been the best team I have had to do a group project with. All team members have been engaged in the tasks allocated to them, have completed the tasks by the deadlines, worked proactively and at no point did anyone drop the ball in any way. The level of communication has also been extremely positive with the chat board being used by all team members on a regular basis and all members present and engaged during team meetings. Any challenges we faced as a team (most notably, losing two members early on) where mitigated quickly by the team maintaining a high level of organization and excellent communication 

*Kym Reilly* 

Officially the first group assignment that I have been a part of. Realizing that often the idea of a group task is highly promoted by those doing the assigning but does not sit well with members completing the task. This is often due to a bad experience stemming from a previously encountered group. With this knowledge at hand, and it being addressed if one of our foremost group meetings, I personally was able to ensure that I was not notably absent. 

As a group, what seemed like one of the hurdles for us was time zones. Once a meeting is scheduled, the perks of MS Teams convert the scheduled time to local time zones. It was just the initial scheduling that seemed to give rise to some issues but was quickly overcome. Only other hiccup that took addressing midway through the task was having the exodus of 2 of 6 members. Fortunately, it was quite early in the planning process and it was easy enough for us to adjust to the new task delegation. 

One of the main strengths of our team was regular correspondence. Having two weekly meetings scheduled early on within the assignment, it meant that we were able to effectively avoid scheduling conflicts, allowing us all to be present for these.  In between these meetings, any questions and issues were addressed in the group forum, allowing people to see and address what they need to when they are available. 

*Phimonrat Panaphong (Belle)*  

The team has formed in the second week of the term, and we were six of us when we started. Then we lost two on the way, but that didn't affect our performance or work quality. We communicate thought MS teams and set the meeting twice a week. Throughout the session, I figured that one of the challenging points is the time zone, as one of the team members left because of that reason. However, after us four remainings, we keep our communication on the MS team chats and emails. I found that working with fewer people, probably a better and efficient way to work. Our meeting always ends smoothly without conflicts, making the team members feel comfortable and perform their best. I used to believe that working in a group can lead to conflicts, miscommunications, and intricacy; however, after acknowledging this team member's input, engagement, and interest in attending the meeting, my perception has changed. The journey that I experience in the past few weeks was incredibly positive, and grateful for our team member. Finally, we are successfully putting together the communication, performance, and devotedness to forming this assignment.

![](images/productive-team-building-activities-blog-header-1.png)

### Group Reflections  
 
All members noted that losing 2 team members was not an ideal situation and at the start of the assignment there were challenges arranging suitable times for all members to attend due to varying Time zones. However, fortunately these issues equated to minor inconveniences and were able to be overcome quickly with good communication and team members prioritizing the assignment. Setting up regular biweekly team meetings, having a detailed action list and splitting tasks up ensured that the team-maintained focus, finished tasks in a timely manner and each team member understood their requirements well. All team members were engaged throughout the assignment, with everyone present during our regular team meetings. All members also actively participated during meetings. In addition to meetings, all team members actively checked and contributed to the Chat board on a regular basis which meant any issues were dealt with in a timely manner.  

At an organisational level tasks were split up to avoid confusion and ensure ownership over certain components of the assignment. This worked particularly well and was tracked with the Action List which allowed us to also determine our contribution percentages and ensure we shared the load evenly. This also allowed us to complete the work to a high standard as we avoided doubling up on tasks and could focus on quality of our answers.  

Unfortunately, the team did not engage with Github on a regular basis, given the ease, familiarity and superior Communication tools afforded by Microsoft teams, all documents were uploaded to Teams. This allowed multiple users to edit the same document at the same time and ensured we all noted new edits in real time. Setting up our Group Page was allocated to one person (a necessity) however most engagement with Github was limited to copying work over from the Microsoft Teams group files. Perhaps in the next assignment the team can collectively engage more with the GitHub repository to gain better exposure to the site and share the coding tasks 
...
| Time Spent in Current Position | Joined a week ago |
| Industry | IT/Creative Agency |
| What Skills, Qualifications and Experience did you need for this role? | Web development experience, multiple backend languages, Hosting/Domain access |
| Main Tasks Performed in role | Web development |
| What is most time spent on? | Coding part of it, using different languages e.g. HTML, PHP, JQuery |
| What aspects of the role are the most rewarding? | When you work on such roles its always rewarding, coz you will be the last person who will launch the project | 
| What aspects of the role are the most Challenging? | When it comes to combability across different platforms (like iPad, mobiles, laptops) to manage the same UI/UX it is a big challenge |
| Can you provide an Example of the work you do that best captures the essence of the IT industry? | BANTA.CO |
| **KEY STAKEHOLDERS** ||
| Key Internal Stakeholders | Teammates, managers |
| Key External Stakeholders | N/A in this role. |
| Is there Community Engagement needed in the role? | Yes, you must discuss the other aspects of the web as well to know what going on the other end, how useful these products are these days |
| What are your general interactions with other IT professionals like? | Normally we hate taking about work, but when we face challenges which are unknown then it is a big plus always to have chat with friends |
